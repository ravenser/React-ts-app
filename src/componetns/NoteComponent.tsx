...
const NoteComponent: React.FC<NoteComponentProps> = ({ index }) => {
  const { allNotes, editNotes, deleteNote, moveToArchive } = useNoteContext();
  ...
  return (
    <tr className="table-row">
      ...
      <td>{!isEditing && <h2 className="name">{allNotes[index].name}</h2>}</td>
      <td>
        <FormatDate date={allNotes[index].created} />
      </td>
      <td>{!isEditing && <p>{allNotes[index].category}</p>}</td>
      <td>{!isEditing && <p>{allNotes[index].content}</p>}</td>
